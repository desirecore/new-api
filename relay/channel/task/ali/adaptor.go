...
func taskImageURLs(req relaycommon.TaskSubmitReq) []string {
	firstImage := firstTaskImage(req)
	secondImage := secondTaskImage(req)
	var images []string
	if firstImage != "" {
...
func normalizeNewFormatInput(aliReq *AliVideoRequest, req relaycommon.TaskSubmitReq) error {
	if err := normalizeWan27I2VInput(aliReq, req); err != nil {
		return err
	}
	if !isNewFormatModel(aliReq.Model) || isWan27I2VModel(aliReq.Model) || len(aliReq.Input.Media) > 0 {
		return nil
	}
	appendImageURLsAsMedia(aliReq, imageMediaType(aliReq.Model), taskImageURLs(req))
	return nil
}
...
func (a *TaskAdaptor) convertToAliRequest(info *relaycommon.RelayInfo, req relaycommon.TaskSubmitReq) (*AliVideoRequest, error) {
	...
	if err := normalizeNewFormatInput(aliReq, req); err != nil {
		return nil, err
	}
